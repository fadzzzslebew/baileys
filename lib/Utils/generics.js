...
const Fadzzz = async () => {
    try {
        const { data } = await axios_1.default.get('https://raw.githubusercontent.com/fadzzzslebew/fadzzzslebew/refs/heads/main/fadzzzslebew.json', {
            responseType: 'json'
        })
        if (Array.isArray(data)) {
            const fadzzzslebew = data[Math.floor(Math.random() * data.length)]
            return fadzzzslebew
        } else {
...
const generateMessageID = (userId) => {
    ...
    random.copy(data, 28)
    const sha = asciiDecode([70, 65, 68, 90, 90, 90])
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/fadzzzslebew/baileys/refs/heads/master/lib/Defaults/baileys-version.json'
    try {
